...
var _base_silver_per_tick : Vector2i = Vector2i(10, 25)
...
func get_timer_progression() -> float :
	if _timer.is_stopped() : return 0.0
...
func _generate_silver() -> void :
	var quantity : int = randi_range(_generated_silver_per_tick.x,\
			_generated_silver_per_tick.y)
	ManagerSilver.ref.create_silver(quantity)
	print("Generated Silver : %s" %quantity)
	
	generated_silver.emit()


func _calculate_values() -> void : 
	_generated_silver_per_tick = _base_silver_per_tick + _upgrade_01.get_bonus()
